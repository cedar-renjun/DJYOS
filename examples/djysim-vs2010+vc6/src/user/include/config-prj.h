...
/*____与硬件相关的配置____*/
#define M 1000000
#define cfg_mclk (400*M)  //主频
#define cfg_hclk (cfg_mclk/4)  //高速外设时钟
#define cfg_pclk (cfg_mclk/8)  //低速外设时钟
#define cfg_timer_clk cfg_pclk  //定时器时钟源

/*____以下定义tick参数____*/
#define cfg_tick_us 10000  //操作系统内核时钟脉冲长度，以毫秒为单位。
#define cfg_tick_hz 100  //内核时钟频率，单位为hz。
#define cfg_fine_us 0x27100000  //40nS,精密时钟脉冲长度，以uS为单位，32位定点数整数、小数各占16位，这也限制了ticks最长不超过65535uS
#define cfg_fine_hz 100  //内核精密时钟频率，是cn_fine_ns的倒数。
...
#define cfg_events_limit 20  //事件数量
...
#define cfg_mmu_used 0  //使用MMU
#define cfg_cache_used 0  //使能Cache
...
#define cfg_main_stack 2048  //main函数所需栈容量
...
#define cfg_single_touch 1  //是否支持单点触摸屏
#define cfg_muti_touch 0  //是否支持多点触摸屏
#define cfg_mouse_2d    1   //是否包含2d鼠标驱动
#define cfg_mouse_3d    1   //是否包含3d鼠标驱动
#define cfg_muti_keyboard 0  //0=不支持多键盘，即使系统拥有多键盘，所有键盘也不加区分地使用
#define cfg_muti_mouse_2d 1  //0=不支持多鼠标，即使系统拥有多个鼠标，所有鼠标也共享同一个鼠标指针。
#define cfg_muti_mouse_3d 0  //0=不支持多3d鼠标，即使系统拥有多个3d鼠标，所有鼠标也共享同一个鼠标指针。
#define cfg_stdin_device "windows_cmd"  //标准输入设备
