...
extern ptu32_t module_init_lcd(ptu32_t para);
...
void prj_module_init(void)
{
    ...
    struct display_rsc *lcd_windows;
#endif
    ...
    module_init_keyboard_windows(0);
#endif

//初始化gui kernel模块
#if ((cfg_gui_kernel == 1) && (cfg_run_mode != cn_mode_tiny))
    module_init_gk(0);
    lcd_windows = (struct display_rsc*)module_init_lcd((ptu32_t)"sim_display");
    
    gk_api_create_desktop(lcd_windows,&desktop,0,0,
...
//以下初始化字体驱动程序
	...
	if(char_term_lhdl != NULL)
    {
        ...
        dev_ctrl_left(char_term_lhdl,cn_cmd_connect_recv_evtt,
...
void run_module_init(void)
{
    ...
    module_init_touch(0);
    module_init_touch_windows((ptu32_t)"sim_display");
#endif
}
