...
//    pg_gk_server_sync = semp_create(1,0,"gk server sync");
    ...
    u16g_gk_server_evtt = djy_evtt_regist(enum_correlative,1,0,0,1,gk_server,
    ...
    semp_set_sync_sort(pg_syscall_semp,sort_fifo);
    djy_event_pop(u16g_gk_server_evtt,NULL,0,NULL,0,0,0);
...
struct gkwin_rsc *gk_create_win(struct gksc_para_create_gkwin *para)
{
    ...
    *gkwin = *parent;             //新窗口拷贝父窗口的属性
    ...
    size = strnlen(para->name,cn_gkwin_name_limit+1);
    ...
    {//父窗口有子窗口
        ...
        {//同级窗口中存在和gkwin的prio相等的窗口
            ...
            if(rsc_is_head(&target_section->node))
                rsc_round_previous(&parent->node);
...
bool_t gk_change_win_area(struct gksc_para_change_win_area *para)
{
    ...
    struct bmbuf_info buf_info;
    struct gkwin_rsc *current,*cwawin;
    ...
    if(cwawin->wm_bitmap.bm_bits != NULL)
    {
        buf_info.vbuf = cwawin->wm_bitmap;
        if( !__gk_vrmalloc(disp,&buf_info,right-left,
                                    bottom-top,cwawin->rop_code))
        {
            djy_error_login(enum_gk_no_memory,"显存不足");
            return false;   //分配显存失败，窗口尺寸未改变(仍然有效)
        }else
        {
            cwawin->change_flag = cn_gkwin_change_none;
            cwawin->wm_bitmap = buf_info.vbuf;
            cwawin->bitmsk = buf_info.rop4_msk;
            cwawin->pat_buf = buf_info.pat;
        }
    }
    if(cwawin == disp->desktop)
    ...
    }else
    {
        ...
        __gk_set_bound(cwawin);                        //设置可显示边界
    }

    current = cwawin;
    current->disp->reset_clip = true;
    current->change_flag = cn_gkwin_change_all;
    while(rsc_trave_scion(&cwawin->node,&current->node))//遍历gkwin的所有子窗口
    {
...
{
    s32 delta_top,delta_left;
    struct gkwin_rsc *current,*movewin;
    struct display_rsc *disp;
    movewin = para->gkwin;
    ...
    }else
    {
        ...
        movewin->right += delta_left;
        movewin->bottom += delta_top;
        __gk_set_bound(movewin);                        //设置可显示边界
    }
    current = movewin;
    current->disp->reset_clip = true;
    current->change_flag = cn_gkwin_change_all;
    //遍历gkwin的所有子窗口
    while(current = (struct gkwin_rsc*)
                            rsc_trave_scion(&movewin->node,&current->node)) 
    {
        current->absx0 += delta_left;        //修改窗口绝对坐标
        current->absy0 += delta_top;
        __gk_set_bound(current);
...
void gk_set_bound_mode(struct gksc_para_set_bound_mode *para)
{
    ...
    current = para->gkwin;
    //遍历gkwin的所有子窗口
    while( ( current = (struct gkwin_rsc*)rsc_trave_scion
...
void gk_set_prio(struct gksc_para_set_prio *para)
{
    ...
    section_start = (struct gkwin_rsc*)rsc_get_previous(&section_end->node);
    ...
    {// 找到gkwin同级窗口中优先级低于或等于新prio的窗口
        ...
        if((target_section == gkwin) 
                || (target_section == (struct gkwin_rsc*)rsc_get_next(&gkwin->node)))
        {
                ...
                }else                                   //有窗口缓冲
                {
                    do
                    {
                                            ...
                                            gkwin->transparentcolor))
                        {
                            ...
                            y_dst = clip->rect.top;
                            if(gkwin->rop_code == cn_r3_srccopy)
                            {
                                    ...
                                    y_src++)
                                {
                                    ...
                                    for(x_src = clip->rect.left-gkwin->absx0;
                                        ...
                                        x_src++)
                                    {
                                        __gk_copy_pixel_bm(dst,src,x_dst,y_dst,
                                            	            x_src,y_src);
                                        x_dst ++; 
                                    }
                                    y_dst ++;
                            ...
                            else if((gkwin->rop_code & cn_rop_ext_en) != 0)
                            {
                                for(y_src = clip->rect.top-gkwin->absy0;
                                    y_src < clip->rect.bottom-gkwin->absy0;
                                    y_src++)
...
void __gk_redraw_all(void)
{
    ...
    {
        do{
            __gk_get_redraw_clip_all(display_next);//扫描取得新的需重绘的区域
            __gk_output_redraw(display_next);    //重绘
                                                ...
                                                void *param2,u16 size2)
{
    ...
    while(1)
    {
        if((ring_capacity(&tg_gk_chunnel.ring_syscall)
                    - ring_check(&tg_gk_chunnel.ring_syscall)) <(u32)(size1+size2+2))
        {       //管道空闲容量不够
...
void gk_server(void)
{
    ...
    while(1)
    {
        ...
        if(semp_query_free(tg_gk_chunnel.syscall_semp) == 0)
