...
bool_t __gk_get_visible_clip(struct display_rsc *display)
{
    ...
    {
        //备份visible_clip
        tempwin->copy_clip = __gk_copy_clip_link(tempwin->visible_clip);
...
//              tempwin->visible_bak = NULL;
    ...
    {   
        ...
        if((special_win->dest_blend) && (special_win->visible_bak != NULL))
        {
            ...
            while(1)
            {
//                tempclip2 = __gk_copy_clip_link(special_win->visible_bak);
                //取得背景窗口需要重绘的区域
                tempclip1 =
                	__gk_get_cliplink_ints(&tempwin->visible_clip, special_win->visible_bak);

//                __gk_free_clip_queue(tempclip2);
