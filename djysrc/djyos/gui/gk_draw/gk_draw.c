                                ...
                                u32 transparentcolor)
{
    ...
    else if((rop_code & cn_rop_rop4_en) != 0)//rop4功能，bit24~31表示背景操作码
    {
        ...
        {//掩码位图掩码像素为0时，使用背景光栅操作，bit24~31表示背景光栅操作码
        	__gk_copy_pixel_rop3_bm(dst_bitmap,src_bitmap,pat,
...
//----在内存画点---------------------------------------------------------------
//功能: 把一个bitmap上的一个点copy到另一个bitmap，
//参数: dst_bitmap，目标位图
//      src_bitmap，源位图
//      x_dst、y_dst，目标位图中的目标坐标
//      x_src、y_src，源位图中的目标坐标
//返回: 无
//-----------------------------------------------------------------------------
void __gk_copy_pixel_bm(struct st_rect_bitmap *dst_bitmap,
...
//            pf = (u32)(*(u16*)(&dst_bitmap->bm_bits[byte_offset_dst]));            
            //注:因对齐问题，不可把目标地址强制转换成16位指针。
            //画像素
            dst_bitmap->bm_bits[byte_offset_dst] = (u8)pf_src;
            dst_bitmap->bm_bits[byte_offset_dst+1] = (u8)(pf_src>>8);
        }break;
        case 24:
        {
            //目标位图中坐标的字节偏移量
...
//----在内存画点(执行三元光栅操作)---------------------------------------------
//功能: 把一个bitmap上的一个点copy到另一个bitmap，执行三元光栅操作，如果bitmap
//      是逐点alpha的，对alpha域不予理会。
//参数: dst_bitmap，目标位图
//      src_bitmap，源位图
//      pat，pattern指针
//      x_dst、y_dst，目标位图中的目标坐标
...
//返回: 无
//-----------------------------------------------------------------------------
void __gk_copy_pixel_rop3_bm(struct st_rect_bitmap *dst_bitmap,
                                struct st_rect_bitmap *src_bitmap,u8 *pat,
                                s32 x_dst,s32 y_dst,s32 x_src,s32 y_src,
                                u32 rop3_code)
{
...
void gk_set_pixel(struct gksc_para_set_pixel *para)
{
    ...
    struct gkwin_rsc *fb_gkwin,*pixelwin;
    struct st_rect_bitmap *bitmap;
    u32 pf_color;
    s32 offsetx,offsety;
    ...
    if((pixelwin->wm_bitmap.bm_bits != NULL)
    ...
    {   //处理方法:在win buffer中绘图，标志changed_msk
        ...
        __gk_shading_pixel(pixelwin,para->x,para->y);//标志像素的changed_msk
    }else
    {
        ...
        //直接写屏属性为true，不管有无缓冲区，都直接画在screen上
        {
            do{
                ...
                if(!((offsetx<clip->rect.left) || (offsetx>=clip->rect.right)
                    || (offsety<clip->rect.top) ||(offsety>=clip->rect.bottom)))
                {
                    __gk_set_pixel_screen(pixelwin->disp,offsetx,offsety,
                                          para->color,para->rop2_code);
                    break;      //像素已经绘制，退出循环
                }else//像素对应的坐标不在可视域内，指向下一个可视域
                    clip = clip->next;
...
void gk_draw_circle(struct gksc_para_draw_circle *para)//确认
{
    struct st_rect limit;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*cirwin;
    ...
    {   //处理方法:在win buffer中绘图，标志changed_msk
        ...
        __gk_draw_circle_bm(cirwin,&limit,para->x0,para->y0,para->r,
                                    para->color,para->rop2_code);
    }else
    {
        ...
        {
            do{
                __gk_draw_circle_screen(cirwin->disp,&clip->rect,
                    ...
                    float x4,float y4,u32 color,u32 rop2_code)
{
    ...
    {
        __gk_lineto_bm(gkwin,limit,(s32)x1,(s32)y1,(s32)x4,(s32)y4,
                                color,rop2_code);
        return;
    }
    x12 = x1+x2;
                    ...
                    float x4,float y4,u32 color,u32 rop2_code)
{
    ...
    if(((a*x2+b*y2+c)*(a*x2+b*y2+c) < ab) &&((a*x3+b*y3+c)*(a*x3+b*y3+c) < ab))
    {
        __gk_lineto_screen(display,limit,(s32)x1,(s32)y1,(s32)x4,(s32)y4,
                           color,rop2_code);
        return;
    }
    x12 = x1+x2;
...
void gk_Bezier(struct gksc_para_Bezier *para)//确认
{
    ...
    struct st_rect limit;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*bzrwin;
    struct disp_draw *my_draw_fun;
    if((para->rop2_code == cn_r2_nop) || (para->rop2_code > cn_r2_last))
        return;                                         //不执行操作
    bzrwin = para->gkwin;
    ...
    //说明有win buffer，且直接写屏属性为false
    if((bzrwin->wm_bitmap.bm_bits != NULL)
    ...
    {   //处理方法:在win buffer中绘图，标志changed_msk
        ...
        __gk_Bezier_bm(bzrwin,&limit,para->x1,para->y1,para->x2,para->y2,
    ...
    }else
    {
        ...
        {
            do{
                __gk_Bezier_screen(bzrwin->disp,&clip->rect,para->x1+offsetx,
                            para->y1+offsety,para->x2+offsetx,para->y2+offsety,
                            para->x3+offsetx,para->y3+offsety,para->x4+offsetx,
                            para->y4+offsety,para->color,para->rop2_code);
                clip = clip->next;
...
void gk_lineto(struct gksc_para_lineto *para)//确认
{
    struct st_rect limit;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*lintowin;
    struct disp_draw *my_draw_fun;
    s32 offsetx,offsety;
    if((para->rop2_code == cn_r2_nop) || (para->rop2_code > cn_r2_last))
        return;                                         //不执行操作
    ...
    //说明有win buffer，且直接写屏属性为false
    if((lintowin->wm_bitmap.bm_bits != NULL)
    ...
    {   //处理方法:在win buffer中绘图，标志changed_msk
        ...
        {    //硬件加速不支持直线绘制，改用软件实现，软件画线算法中
            //同时处理了changed_msk
            __gk_lineto_bm(lintowin,&limit,para->x1,para->y1,para->x2,
                                    para->y2,para->color,para->rop2_code);
        }
        else    //硬件加速绘图时，未处理changed_msk，处理之
        {  
            __gk_shading_line(lintowin,&limit,
        ...
        //直接写屏属性为true，不管有无缓冲区，都直接画在screen上
        {
            do{//在screen上直接画直线，不画终点
            	if(!my_draw_fun->line_screen(&clip->rect,para->x1+offsetx,
                         	...
                         	para->color,para->rop2_code))
                {
                	__gk_lineto_screen(lintowin->disp,&clip->rect,
                            para->x1+offsetx,para->y1+offsety,para->x2+offsetx,
                            para->y2+offsety,para->color,para->rop2_code);
                }
                clip = clip->next;
...
void gk_lineto_ie(struct gksc_para_lineto *para)//确认
{
    ...
    struct st_rect limit;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*lintoiewin;
    struct disp_draw *my_draw_fun;
    ...
    {
        //说明有win buffer，且直接写屏属性为false
        if((lintoiewin->wm_bitmap.bm_bits != NULL)
        ...
        {   //处理方法:在win buffer中绘图，标志changed_msk
            ...
            {    //硬件加速不支持直线绘制，改用软件实现，软件画线算法中
                //同时处理了changed_msk
            	__gk_hlineto_bm(lintoiewin,&limit,para->x1,para->y1,
                            	para->x2+1,para->color,para->rop2_code);
            }
            else
            {   //硬件加速支持直线绘制，未处理changed_msk，处理之
            	__gk_shading_line(lintoiewin,&limit,
	...
	else if(para->x1 == para->x2)    //绘制垂直线
    {
        ...
        {   //处理方法:在win buffer中绘图，标志changed_msk
            ...
            {    //硬件加速不支持直线绘制，改用软件实现，软件画线算法中
                //同时处理了changed_msk
            	__gk_vlineto_bm(lintoiewin,&limit,para->x1,
    ...
    else	//绘制斜线
    {
        ...
        {   //处理方法:在win buffer中绘图，标志changed_msk
            ...
            {    //硬件加速不支持直线绘制，改用软件实现，软件画线算法中
            	...
            	if(dy > dx)
                {
                	__gk_olineto_bm(lintoiewin,&limit,para->x1,para->y1,
                                	para->x2,para->y2+1,
                                	para->color,para->rop2_code);
                }
            	else
...
void gk_draw_bitmap(struct gksc_para_draw_bitmap *para)//确认
{
    ...
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*bitmapwin;
    struct st_rect_bitmap *bitmap;
    struct st_rect_bitmap *src,*dst;    
    s32 x_src,y_src,x_dst,y_dst;
    
    bitmapwin = para->gkwin;
    bitmap = &bitmapwin->wm_bitmap;
    ...
    //说明有win buffer，且直接写屏属性为false
    if((bitmapwin->wm_bitmap.bm_bits != NULL) 
        && (bitmapwin->direct_screen == false))     
    {   //处理方法:在win buffer中绘图，标志changed_msk
        if(!bitmapwin->disp->draw.blt_bm_to_bm(bitmap,&target,
        ...
        {   
                ...
                y_src++)
            {
                ...
                for(x_src = 0;
                    ...
                    x_src++)
                {
                    __gk_copy_pixel_bm(bitmap,&para->bitmap,
        ...
        __gk_shading_rect(bitmapwin,&target);    //设置绘制部分的changed_msk
    }
    else
    {
        clip = bitmapwin->visible_clip;
        if(clip == NULL)                            //窗口的可视域为空，直接返回
            return ;
        fb_gkwin = bitmapwin->disp->frame_buffer;
        target.left += bitmapwin->absx0;
        target.right += bitmapwin->absx0;
        target.top += bitmapwin->absy0;
        target.bottom += bitmapwin->absy0;
        //有frame buffer,且直接写屏属性为false
        if((fb_gkwin != NULL) && (bitmapwin->direct_screen == false))     
        {//处理方法:在frame_buffer中绘图，但只绘gkwin中的可视区域
            ...
            do{
                ...
                clip = clip->next;
            }while(clip != bitmapwin->visible_clip);
        }
        else       
        //无win buffer，也无frame buffer，直接画在screen上
        //直接写屏属性为true，不管有无缓冲区，都直接画在screen上
        {
            ...
            do{
                ...
                {
                    //传送bitmap到screen
                    __gk_blt_bm_to_screen(bitmapwin->disp,&ins_rect,
                                        &para->bitmap,
...
//    bitmap_para.command =cn_gksc_draw_bitmap;
    ...
    for(; src_len > 0;)
    {
        len= djy_mbtowc(&wc, text, src_len);
        if(len == -1)
        { // 无效字符
            src_len--;
            text++;
...
void gk_draw_bitmap_rop(struct gksc_para_draw_bitmap_rop *para)//确认
{
    ...
    //说明有win buffer，且直接写屏属性为false
    if((bitmap->bm_bits != NULL) && (bitmapropwin->direct_screen == false))     
    {   //处理方法:在win buffer中绘图，标志changed_msk
            ...
            else//给定的光栅操作码为三元光栅操作码
            {
                    ...
                    y_src++)
                {
                    ...
                    for(x_src = 0;
                        ...
                        x_src++)
                    {
                        __gk_copy_pixel_rop3_bm(dst,src,pat.bm_bits,
                            x_dst,y_dst,x_src,y_src,para->rop_code);
                        x_dst++;                                                                        
                    }
                    y_dst++;                                                                        
        ...
        __gk_shading_rect(bitmapropwin,&target);    //设置绘制部分的changed_msk
    }
    else
    {
        clip = bitmapropwin->visible_clip;
        if(clip == NULL)                            //窗口可视域为空，直接返回
            return ;
        fb_gkwin = display->frame_buffer;
        //有frame buffer，且直接写屏属性为false
        if((fb_gkwin != NULL) && (bitmapropwin->direct_screen == false))     
        {   //处理方法:在frame_buffer中绘图，但只绘gkwin中的可视区域
            //为与可视域坐标保持一致，将待位图要绘制的位置的
            //坐标调整成以screen原点为原点的坐标
            target.left += bitmapropwin->absx0;
            target.right += bitmapropwin->absx0;
            target.top += bitmapropwin->absy0;
            target.bottom += bitmapropwin->absy0;
            do{
                if(__gk_get_rect_ints(&clip->rect,&target,&ins_rect))
                {
                    if(!fb_gkwin->disp->draw.blt_bm_to_bm(
                        ...
                        else//给定的光栅操作码为三元光栅操作码
                        {
                            	...
                            	y_src++)
                            {
                                ...
                                for(x_src = ins_rect.left-target.left;
                                	...
                                	x_src++)
                                {
                                	__gk_copy_pixel_rop3_bm(dst,src,
                                        	pat.bm_bits,x_dst,y_dst,x_src,
                                            y_src,para->rop_code);
                                	x_dst++;                                                                        
                                }
                ...
                clip = clip->next;
            }while(clip != bitmapropwin->visible_clip);
        }
        else       
...
void gk_fill_part_win(struct gksc_para_fill_part_win *para)//确认
{
    ...
    struct disp_draw *my_draw_fun;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*fpwwin;
    ...
    //说明有win buffer，且直接写屏属性为false
    if((fpwwin->wm_bitmap.bm_bits != NULL)
    ...
    {   
        ...
        __gk_shading_rect(fpwwin,&para->rect);//着色填充区域的changed_msk
    }else
    {
        ...
        //直接写屏属性为true，不管有无缓冲区，都直接画在screen上
        {
            do{
                ...
                if(__gk_get_rect_ints(&clip->rect,&target,&ins_rect))
                {
                    ...
                    mirror = (struct display_rsc *)&fpwwin->disp->node;
                    if(mirror == NULL)      //没有镜像显示器，直接返回
                        return;
                    current = mirror;
                    while(current != NULL)
...
void gk_fill_win(struct gksc_para_fill_win *para)//确认
{
    ...
    struct st_rect rect;
    struct clip_rect *clip;
    struct gkwin_rsc *fb_gkwin,*fpwin;
    struct st_rect_bitmap *bitmap;
    ...
    //说明有win buffer，且直接写屏属性为false
    if((fpwin->wm_bitmap.bm_bits != NULL)
    ...
    }else
    {
        ...
        //直接写屏属性为true，不管有无缓冲区，都直接画在screen上
        {
            do{
                ...
                mirror = (struct display_rsc *)&fpwin->disp->node;
                if(mirror == NULL)      //没有镜像显示器，直接返回
                    return;
                current = mirror;
                while(current != NULL)
