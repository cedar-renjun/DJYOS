...
static struct mem_cell_pool tg_pool_of_cell_pool[cfg_mem_pools];
...
ptu32_t module_init_memb(ptu32_t para)
{
    ...
    cell_pool.pool_offset = (ptu32_t)&tg_pool_of_cell_pool[cfg_mem_pools];
    ...
    semp_create_knl(&cell_pool.memb_semp,
                      cfg_mem_pools,cfg_mem_pools,"固定块分配池");
    return 1;
}
...
void *mb_malloc(struct mem_cell_pool *pool,u32 timeout)
{
    ...
    if(semp_pend(&pool->memb_semp,0) == false)  //无信号量，表明内存块已经用完
    {
        ...
        if(semp_query_capacital(&pool->memb_semp) < pool->cell_limit)
        {
            ...
            inc = m_malloc_gbl(pool->cell_increment*pool->cell_size
                                + align_up_sys(1),0);
            if(inc != NULL)
            {
                //检查实际分配到的内存量
                inc_size = m_check_size(inc) - align_up_sys(1);
