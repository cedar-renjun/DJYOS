...
#define cn_evtt_id_mask     0x4000      //相与可得事件类型表中的偏移
...
enum knl_error_code
{
    ...
    enum_knl_vpu_over,              //事件类型的线程超过限制数
    ...
    enum_knl_stack_overflow,        //栈溢出风险警告
};
...
struct  thread_vm        //线程数据结构
{
    //线程栈指针,在线程被抢占时保存sp,运行时并不动态跟踪sp变化
    u32    *stack;
    u32    *stack_top;     //线程的栈顶指针
    struct thread_vm *next;     //用于把evtt的所有空闲线程连成一个单向开口链表
                                //该链表由evtt的my_free_vm指针索引
    u32    stack_size;     //栈深度
    struct process_vm *host_vm;  //宿主进程，在si和dlsp模式中为NULL
};
...
{
    u16 event_delay:1;          //闹钟同步
    u16 wait_overtime:1;        //超时等待
    u16 event_sync:1;           //事件同步
    u16 event_sync_exit:1;      //事件同步中被同步的目标事件异常退出
    u16 evtt_pop_sync:1;        //事件类型弹出同步
    u16 evtt_done_sync:1;       //事件类型完成同步
    u16 wait_memory:1;          //从系统堆分配内存同步
    u16 wait_semp:1;            //信号量同步
    u16 wait_mutex :1;          //互斥量同步
    u16 wait_asyn_signal:1;     //异步信号同步
    u16 wait_cpipe:1;           //定长pipe
    u16 wait_vpipe:1;           //动态长度pipe
    u16 wait_msg_sent:1;        //等待消息发送
    u16 wait_msg_completed:1;   //等待消息处理完成
};
...
#define cn_db_info_stack        3
#define cn_db_info_lock         4


#define cn_creat_new_para   0x01    //为关联型事件创建新消息
...
struct event_ECB
{
    ...
    struct  thread_vm  *vm;             //处理本事件的线程指针,
    struct  para_record *para_high_prio;//高优先级参数队列
    struct  para_record *para_low_prio; //低优先级参数队列
    struct  para_record *para_current;  //当前将要或正在处理的参数。
...
struct event_type
{
    ...
    u16    events;     //分配的事件总数
    u16    vpus_res;   //系统繁忙时为本类型事件保留的线程数量，关联型无效
...
#define djy_task_completed(x) djy_echo_para(x)  //用于兼容老版本
