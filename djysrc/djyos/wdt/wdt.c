...
#if(cfg_wdts_limit != 0)
static struct wdt_rsc tg_wdt_rsc_pool[cfg_wdts_limit];
#endif
...
static u32 wdt_check_timeout;       //看门狗事件执行周期
static u32 wdt_hard_timeout = cn_limit_uint32;  //硬件看门狗定时器溢出时间。
...
ptu32_t module_init_wdt(ptu32_t para)
{
...
#if(cfg_wdts_limit != 0)
    pg_wdt_rsc_pool = mb_create_pool(&tg_wdt_rsc_pool,cfg_wdts_limit,
                                sizeof(struct wdt_rsc),0,0,"wdt pool");
#else
    pg_wdt_rsc_pool = mb_create_pool(NULL,cfg_wdts_limit,
                                sizeof(struct wdt_rsc),0,0,"wdt pool");
#endif
    //建立根资源节点
    ...
    u16g_wdt_evtt = djy_evtt_regist(enum_correlative,cn_prio_wdt,0,0,1,wdt_check,
                                100,"wdt service");
    if(u16g_wdt_evtt == cn_invalid_evtt_id)
        return 0;
	djy_event_pop(u16g_wdt_evtt,NULL,0,NULL,0,0,0);
	return 1;
}
...
//----设置硬件溢出周期---------------------------------------------------------
//功能: 设置硬件狗的溢出周期，有硬件看门狗的系统，在硬件看门狗初始化模块中，可
//      调用本函数把看门狗的溢出时间告诉系统。
//参数：hard_timeout，硬件狗叫时间，单位:us
//返回：无
//-----------------------------------------------------------------------------

//----设置看门狗事件执行周期----------------------------------------------------
//功能: 设置看门狗事件执行周期，用于检查小狗是否狗叫。利用硬件狗叫
...
void wdt_set_check_timeout(u32 hard_timeout)
{
    ...
    wdt_check_timeout = min*7/8;//使喂狗时间间隔小于狗叫时间
}
...
void wdt_check(void)
{
    ...
    while(1)
    {
        //每check一次，计算一次check_timeout,因为看门狗随时会新增、删除
