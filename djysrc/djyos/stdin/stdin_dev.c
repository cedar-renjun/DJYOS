...
s32 stdin_install_device(char *device_name,enum stdin_input_type stdin_type,
                         void *private)
{
    struct stdin_device_rsc *djy_stdin;
    ...
    {
        djy_stdin = mb_malloc(pg_stdin_device_pool,0);
        if(djy_stdin != NULL)
        {
            rsc_add_son(&pg_stdin_device_rsc_tree->stdin_device_node,
                    &djy_stdin->stdin_device_node,sizeof(struct stdin_device_rsc),
                    device_name);
            djy_stdin->input_type = stdin_type;
            djy_stdin->stdin_private = (ptu32_t)private;
            djy_stdin->input_counter = 0;
            djy_stdin->focus_evtt = u16g_default_focus;
            s32g_next_id += 1;
            djy_stdin->device_id = s32g_next_id;
...
bool_t stdin_input_msg(s32 stdin_id,u8 *msg_data, u32 msg_size)
{
    ...
    ((struct stdin_device_rsc *)target)->input_counter++;
    ...
    {
        input_msg.input_data = *(union input_device_mem *)msg_data;
        ...
        while(1)
        {
            if(djy_event_pop(evtt,NULL,0,
                            &input_msg,sizeof(input_msg),2,0)
                            != cn_invalid_event_id)
            {
                break;
            }
...
bool_t stdin_uninstall_device(char *device_name)
{
    ...
    struct stdin_device_rsc *djy_stdin;
    djy_stdin = (struct stdin_device_rsc *)rsc_search_son(
                 &pg_stdin_device_rsc_tree->stdin_device_node,device_name);
    if(djy_stdin == NULL)
        return false;
    ...
    {
        mb_free(pg_stdin_device_pool,djy_stdin);
        return true;
    }
    else
    	return false;
