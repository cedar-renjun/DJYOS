...
//说明： cfg_locks_limit 是用户配置的，由于用户并不知道操作系统需要用多少信号量，
//      所以操作系统并不占用 cfg_locks_limit 指标，用户使用的信号量从
//      tg_semp_pool定义的内存池中分配，操作系统使用的信号量自己定义，两不
//      相关，如果用户企图调用 semp_delete 删除操作系统定义的信号量，将引起造成
//      内存泄漏，而且相关操作系统资源也将失去信号量保护，后果不可预料
//定义信号量结构初始内存池
#if(cfg_locks_limit != 0)
static union lock_MCB tg_lock_mem_block[cfg_locks_limit];
#endif
struct mem_cell_pool *pg_lock_pool;  //信号量结构内存池头指针
...
//        event->event_status.bit.wait_mutex = 0; //清除等待状态
        ...
        {
        	if(pg_event_running->prio == event->prio)
        	{
...
bool_t mutex_pend(struct mutex_LCB *mutex,u32 timeout)
{
    ...
    if(mutex == NULL)
        return true;
    if(pg_event_running == mutex->owner)
        return true;
    //以下读取信号灯数量，必须保证原子操作，故关中断
    atom_low = int_low_atom_start(  );             //本函数对不能嵌套调用
