...
//fscanf, djyfs_fwrite, getc, getchar, gets, djy_printf, putc, putchar, puts,
...
//int djy_printf(const char * restrict format, ...);
...
static struct file_rsc tg_content_buf[cfg_opened_fsnode_limit];//定义目录项内存池
...
//      文件柜第一个根目录。cfg_work_path，在cofig-prj.h里配置
//参数: 
//返回: 1，设置成功
//      0, 设置失败
//-------------------------------------------------------------------------
...
bool_t print_work_path(void)
{
    ...
    djy_printf("%s:\\", point_num[index]->name);
    //全路径需要。比如"C:\\"
...
//-----------------------------------------------------------------------------
bool_t __djyfs_DBX_lookfor_item(struct pan_device *DBX_lhdl,
                                          char *synname,
                                          struct file_rsc *result)
{
    struct st_DBX_device_tag *DBX_device_tag;
    struct file_rsc *parent,*son=NULL,fp_buf;
    uint16_t next_char_off;
    char    name[cn_file_name_limit+1];
    DBX_device_tag = (struct st_DBX_device_tag*)dev_get_my_tag(DBX_lhdl);
    if(__djyfs_is_abs_path(synname))  //synname中已经去掉了文件柜名，无需再判断
    {
    ...
    //已经判断文件柜存在了，所以遇到只有"\\"的路径，就说明根目录存在
   	if(strcmp(synname,"\\")==0)
   	{
    	if(result != NULL)
            memcpy(result,DBX_device_tag->opened_root,sizeof(struct file_rsc)); 
        return true;          //返回的是根目录
   	}
    ...
    {//沿路径逐级搜索目录和文件
        ...
        {//目标文件(目录)尚未打开
            rsc_clean_node(&fp_buf.file_node);
            //查找目标并初始化文件数据结构
            if(DBX_device_tag->lookfor_item(name,parent,&fp_buf))
            {//目标存在，并且已经初始化目录信息
    ...
    {
        if(son != &fp_buf)
        	memcpy(result,&fp_buf,sizeof(struct file_rsc)); 
        else
        	memcpy(result,son,sizeof(struct file_rsc)); 
    }
    return true;
}
    ...
    switch(right_cmd)
    {
        ...
        //的信号量。但是，文件系统不应该访问medium_tag成员，故用data2传入。
            ...
            if(DBX_device_tag->formatted)   //这是已经格式化好的文件柜
            {
                ...
                root_folder->attr.bits.folder = 1;
                root_folder->attr.bits.read_only = 1;
                //文件柜设备的打开文件的根结点指向该根资源结点。
                DBX_device_tag->opened_root = root_folder;
                strcat(DBX_device_tag->opened_root->name,DBX_device_tag->name);
                strcat(DBX_device_tag->opened_root->name,":");
            }else                           //这是未格式化的文件柜
...
//----------------------------------------------------------------------------
uint16_t __djyfs_is_contain_DBX_name(char *fullname)
{
    uint16_t offset;
...
//备注: 内部函数，调用前先确保fullname是合法的字符串。
//----------------------------------------------------------------------------
bool_t __djyfs_is_abs_path(char *fullname)
{
    if(fullname[0] == '\\')
    {
        return true;
    ...
    {
        if(__djyfs_is_contain_DBX_name(fullname) != 0)
            return true;
        else
            return false;
    }
...
//备注: 内部函数，调用前先确保fullname是合法的字符串。
//----------------------------------------------------------------------------
bool_t __djyfs_is_contain_filename(char *fullname)
{
...
//----------------------------------------------------------------------------
bool_t __djyfs_item_is_opened(struct st_DBX_device_tag *DBX_device_tag,
                              char *synname)
{
    struct file_rsc *parent,*son;
    ...
    if(__djyfs_is_abs_path(synname))
    {
        parent = DBX_device_tag->opened_root;  //从根目录开始操作
        offset = 1;
    }else
...
uint32_t __djyfs_DBX_remove_item(struct pan_device *DBX_lhdl,char *synname)
{
    ...
    struct st_DBX_device_tag *DBX_device_tag;
    struct file_rsc *parent,*son=NULL;
    uint16_t next_char_off;
    char    name[256];
    DBX_device_tag = (struct st_DBX_device_tag*)dev_get_my_tag(DBX_lhdl);

    if(__djyfs_item_is_opened(DBX_device_tag,synname) == true)
    {
                            ...
                            char *old_synname,char *newname)
{
    ...
    if(__djyfs_item_is_opened(DBX_device_tag,old_synname) == true)
    {
    	return (uint32_t)enum_fs_object_opened;       //不能删除已经打开的对象
    }
    if(__djyfs_is_abs_path(old_synname))  //synname中已经去掉了文件柜名，无需再判断
    {
        parent = DBX_device_tag->opened_root;  //从根目录开始操作
        next_char_off = 1;
    }else
                                ...
                                uint32_t data1,uint32_t data2)
{
    ...
    switch((enum DBX_left_cmd)left_cmd)
    {
        ...
        {//格式化文件柜，data1、data2是格式化参数
            ...
            {
                DBX_device_tag->opened_root->attr.bits.folder = 1;
                DBX_device_tag->opened_root->attr.bits.read_only = 1;
                strcat(DBX_device_tag->opened_root->name,DBX_device_tag->name);
                strcat(DBX_device_tag->opened_root->name,":");
                DBX_device_tag->opened_root->home_DBX = DBX_lhdl;
        ...
        {//查找文件，data1是文件名(目录名)
            return (ptu32_t)__djyfs_DBX_lookfor_item(DBX_lhdl,
                                                    (char*)data1,
                                                    (struct file_rsc*)data2);
...
djy_handle_t __open_DBX_left(char *fullname)
{
    ...
    name_len = __djyfs_is_contain_DBX_name(fullname);
    if(name_len != 0)
...
//----获取根目录---------------------------------------------------------------
//功能: 根据一个文件指针，取得该文件指针所在的文件柜的根目录指针
//参数: fp，文件指针
//返回: 根目录指针
//-----------------------------------------------------------------------------
struct file_rsc *djyfs_get_rootfolder(struct file_rsc *fp)
{
    struct st_DBX_device_tag *DBX_device_tag;
    //fullname不是一个合法的字符串
    if(fp == NULL)
        return false;
    DBX_device_tag = (struct st_DBX_device_tag*)dev_get_my_tag(fp->home_DBX);
    if(DBX_device_tag->formatted)
    {
        return DBX_device_tag->opened_root;
    }else
        return NULL;
}
//----取文件柜信息-------------------------------------------------------------
//功能: 取一个文件柜的信息，总空间，空闲空间等
//参数: fullname，文件柜名
//      sum,valid,free,用于返回数据，可以是NULL
//返回: true = 正确获取，false = 错误，可能参数错误，也可能没格式化
//-----------------------------------------------------------------------------
bool_t djyfs_dbx_stat(char *fullname,s64 *sum,s64 *valid,s64 *free)
{
    bool_t result;
    djy_handle_t DBX_lhdl;
    struct st_DBX_device_tag *DBX_device_tag;
    //fullname不是一个合法的字符串
    if(__check_fullname(fullname) == false)
        return false;
    ...
    if(DBX_device_tag->formatted)
    {
        DBX_device_tag->check_DBX(DBX_device_tag,sum,valid,free);
        result = true;
    }else
        result = false;
    dev_close_left(DBX_lhdl);
...
//----取文件柜信息-------------------------------------------------------------
//功能: 取fp所在的文件柜的信息，总空间，有效弓箭、空闲空间
//参数: fp，文件指针
//      sum,valid,free,用于返回数据，可以是NULL
//返回: true = 正确获取，false = 错误，可能参数错误，也可能没格式化
//-----------------------------------------------------------------------------
bool_t djyfs_dbx_stat_fp(struct file_rsc * fp,s64 *sum,s64 *valid,s64 *free)
{
    bool_t result;
    struct st_DBX_device_tag *DBX_device_tag;
    //fullname不是一个合法的字符串
...
//返回: 字符串长度，含串结束符'\0'
//-----------------------------------------------------------------------------
u32  djyfs_getpath_len(struct file_rsc * fp)
{
    ...
    DBX_device_tag = (struct st_DBX_device_tag *)(fp->home_DBX
                                                            ->private_tag);
    ...
    for(index = 0; index<cn_path_depth_limit; index++)
    {
        ...
        len++;      //字符'\'，以及最后一级的'\0'的位置
        if(current_fp == DBX_device_tag->opened_root)
            break;
...
//-----------------------------------------------------------------------------
bool_t djyfs_getpath(struct file_rsc * fp,char *buf,s32 maxlen)
{
    struct file_rsc *current_fp;
    struct st_DBX_device_tag *DBX_device_tag;
    uint32_t index=0,completed = 0;
    struct rsc_node *point_num[cn_path_depth_limit];
    ...
    if(buf == NULL)
    {
        maxlen = djyfs_getpath_len(fp);
        if((buf = m_malloc(maxlen,0)) == NULL)
            return false;
        buf[0] = '\0';
    }
    ...
    for( ; index>0; index--)
    {
        if(completed + strlen(point_num[index-1]->name) < (u32)maxlen)
            strcat(buf,point_num[index-1]->name);
    }
    return true;
}
...
//返回: 字符串长度，含串结束符'\0'
//-----------------------------------------------------------------------------
u32  djyfs_getcwd_len(void)
{
    return djyfs_getpath_len(pg_work_path);
}
...
//      maxlen，长度限制，含串结束符'\0'。
//返回: true = 成功获取，false = 失败
bool_t djyfs_getcwd(char *buf,s32 maxlen)
{
    if(maxlen == 0)
        return false;
    if(buf == NULL)
    {
        maxlen = djyfs_getcwd_len( );
        if((buf = m_malloc(maxlen,0)) == NULL)
            return false;
        buf[0] = '\0';
    }
    return djyfs_getpath(pg_work_path,buf,maxlen);
}
...
//      api的时候再说。
//-----------------------------------------------------------------------------
djyfs_file *djyfs_folder_traversal_son(struct file_rsc *parent,
                                        	struct file_rsc *current)
{
    ...
    if(parent == NULL)
        return NULL;
    DBX_device_tag = (struct st_DBX_device_tag *)(parent->home_DBX
                                                            ->private_tag);
    if(DBX_device_tag->formatted)
    {
        //调用文件柜的左手控制函数
        result = DBX_device_tag->item_traversal_son(parent,current);
    }else
        result = NULL;
    return result;
}
...
//返回: 找到则返回true，找不到返回false
//-----------------------------------------------------------------------------
bool_t djyfs_fsearch(char *fullname,struct file_rsc* buf)
{
    bool_t result;
    djy_handle_t DBX_lhdl;
    struct st_DBX_device_tag *DBX_device_tag;
...
bool_t djyfs_format(uint32_t format_para1,uint32_t format_para2,char *dbx_name)
{
    ...
    if(dbx_name == NULL)
        return false;
    name_len = strnlen(dbx_name,cn_DBX_name_limit+1);
    if((name_len == 0) || (name_len > cn_DBX_name_limit))
        return false;
