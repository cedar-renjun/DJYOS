...
void __DFFSD_rebuild_DDR_main(struct st_DBX_flash_tag *DBX_flash_tag)
{
    ...
    bool_t *writed = DBX_flash_tag->writed_DDR_main; //指向是否完成写入的指针
    uint32_t *DBL_buf;
    DDR_blocks = (DBX_flash_tag->DDR_size + chip->block_size -1)
                                / chip->block_size;    //计算DDR表占用的块数
    ...
    do
    {
        ...
        for(loop = 0; loop < DDR_blocks; loop++)
        {
            ...
            while(offset - completed < write_len)
            {
                ...
                {
                    offset = MAT_start;
                }else if(offset <= DBX_flash_tag->DDR_size)//写入MAT链表
...
void __DFFSD_sort_ART(struct st_DBX_flash_tag *DBX_flash_tag)
{
    ...
    pos_tab = DBX_flash_tag->ART_position;      //磨损位置表
    times_tab = DBX_flash_tag->ART_times;       //磨损次数表
    invalid_offset = DBX_flash_tag->block_sum;  //坏块位置指向磨损表底部
    loop = invalid_offset;
    do  //第一遍扫描，把全部坏块移到磨损表的末端
    {
        loop--;
        block_no = pos_tab[loop];
        if(DBX_flash_tag->DDR_MAT[block_no].attr == cn_invalid_block) //坏块
        {
            invalid_offset--;
