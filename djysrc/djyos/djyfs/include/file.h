...
struct st_DBX_device_tag
{
    ...
    struct file_rsc *opened_root;   //打开的根目录项，名字=文件柜名
    ...
    //查找文件(目录)，result非空则返回item信息，但不分配读写
	...
	struct file_rsc *(*item_traversal_son)(struct file_rsc * parent, 
...
bool_t __djyfs_item_is_opened(struct st_DBX_device_tag *DBX_device_tag,char *synname);
...
bool_t djyfs_fsearch(char *fullname,struct file_rsc *fp_buf);
