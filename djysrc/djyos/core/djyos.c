    ...
    while(1)
    {
        ...
        if(event_para != NULL)
        {
            switch(*event_para)
            {
                ...
                {
                    djy_printf("事件号  所属类型  使用   优先级 CPU  内存\n\r");
                ...
                {
                    djy_printf("类型号  登记   优先级 处理函数    内存   名字\n\r");
                ...
                case cn_db_info_stack:
                {
                    ...
                    for(pl_ecb = 0; pl_ecb < cfg_events_limit; pl_ecb++)
                    {
                        ...
                        {
                            djy_printf("%5d  空闲事件控制块",pl_ecb);
                        }
                        djy_printf("\n\r");
                    }
                    djy_printf("栈指针是最后一次上下文切换时保存的值，溢出风险仅供参考\n\r");
                }break;
                case cn_db_info_lock:
                {
                    ...
                    lock = current_node;
                    djy_printf("信号量 semaphore 列表：\r\n");
                    djy_printf("类型  信号数上限  可用信号数  阻塞事件  名字\r\n");
                    while(1)
                    {
                    ...
                    lock = current_node;
                    djy_printf("\n\r互斥量 mutex 列表：\r\n");
                    djy_printf("状态  拥有者  原优先级  现优先级  阻塞事件  名字\r\n");
                    while(1)
                    {
                        current_node = rsc_trave_scion(lock,current_node);
                        if(current_node == NULL)
                        ...
                        else
                        {
                            ...
                            {
                                djy_printf("%05d   未发生优先级翻转    ",pl_ecb->event_id);
                            }
                        }
                       ...
                       char *evtt_name)
{
    ...
    for(evtt_offset=0; evtt_offset<cfg_evtts_limit; evtt_offset++)   
        if( tg_evtt_table[evtt_offset].property.registered ==0)
            break;
    if(evtt_offset == cfg_evtts_limit)     //没有空闲事件控制块
    {
    ...
    {
        tg_evtt_table[evtt_offset].evtt_name[0] = '\0';   //清空事件类型名
    }
    ...
    {
        tg_evtt_table[evtt_offset].vpus_res = 0;    //关联型事件，vpus_res无效
    }
...
//      2.把 cfg_events_limit 个事件控制块结构连接成一个空闲队列,队列的结构参见
...
void __djy_init_sys(void)
{
    ...
    for(i = 1; i < cfg_events_limit; i++)    //i=0为系统服务事件
    {
...
//    tg_evtt_table[0].property.overlay = 0;
    ...
    {
        djy_error_login(enum_mem_tried,"创建线程时内存不足");
...
u32 djy_wait_evtt_completed(u16 evtt_id,u16 done_times,u32 timeout)
{
    ...
    if(timeout == 0)
        return (u32)cn_sync_timeout;
    int_save_asyn_signal();
    if((pl_evtt->property.registered == false)     //事件类型未注册
...
//      弹出事件时，可以带一个参数，提供处理事件所需要的附加信息。
//参数：hybrid_id，id，可能是事件类型id，也可能是事件id
//          如果是事件id，系统会查找该事件对应的事件类型id。使用事件id对独立型
//          事件非常有用，可以为对应的事件添加参数，也可以选择性地唤醒事件。
...
//          回处理结果。单位是us
//      event_para,事件参数指针，其含义由事件处理函数解析，最好不超过32字节。
//      tip_size，参数长度，最好不超过32.0表示无参数
//      para_options, 参数选项，只有两个bit有用:
//          bit0:1=总是创建新参数到提示队列，0=如果相应队列尚无提示，则
//              创建新参数，否则替代该队列的最后一个提示的内容。关联型才有效
//          bit1:参数内容超过32bytes时的处理方法，
//              0=直接使用调用方提供的缓冲区，此时应用程序应该自行解决内存访问冲
//              突的问题，如果timoout=cn_timeout_forever，则不会有访问冲突问题
//              1=malloc一个内存块。
//      prio,事件优先级,0表示使用默认值(存在事件类型表pg_event_type_table中),对
//          于关联型事件，如果事件队列中已经有事件的话，则0表示维持原事件的优先，
//          级，其他值表示改变成的目标优先级。
                    ...
                    ufast_t prio)
{
    ...
    //  b、如果hybrid_id是事件类型id，则机会所有sync_counter减至0的事件。
    pl_ecb = pl_evtt->pop_sync;
    if(pl_ecb != NULL)
    {
        do
                    ...
                    && (hybrid_id & cn_evtt_id_mask)) )
    {
        if((pg_event_free==NULL)            //没有空闲的事件控制块
            ||((pg_para_free == NULL) && (para_size != 0)))  //需要参数但却没有参数控制块
        {
            ...
            if(para_size != 0)          //有参数
            {
                ...
                if(para_size > 32)
                {
                    if(malloc32 == true)    //动态分配内存，然后copy参数
                    {
                        ...
                        op_queue->event_para = event_para;  //直接使用调用者提供的缓冲区，
                                                //无须copy参数
    ...
    else  //关联型事件，且队列中已经有该类型事件 或 独立型，且hybrid_id是事件id
    {
        ...
        if(para_size != 0)          //有参数
        {
            ...
            if((para_options && cn_creat_new_para)      //创建新参数
                    || (pl_ecb->para_low_prio == NULL)) //参数队列尚无参数
            {
                ...
                }else
                {
                    ...
                    pg_para_free = pg_para_free->next;
                    op_queue->sync = NULL;
                    if(pl_ecb->para_low_prio == NULL)          //原队列空
                    {
    ...
    //设定了超时时间，将重新调度
    //如果参数附带参数，则调用者进入参数的同步队列，阻塞知道该参数所代表的任务
    //完成才返回
    //否则进入事件同步队列，只要目标事件处理函数调用一次djy_task_completed即解除
    //阻塞
    if(timeout != 0)        
    {
...
void *djy_get_event_para(u32 *time)
{
    ...
    }else if(pg_event_running->para_high_prio != NULL) //高优先级参数队列非空
    {
        sub = pg_event_running->para_high_prio;  //取高优先级参数队列
    ...
    }else if(pg_event_running->para_low_prio != NULL)//低优先级参数队列非空
    {
        sub = pg_event_running->para_low_prio;   //取低优先级参数队列
        if(sub->next == sub)                    //队列中只有一个参数
        {
...
#define cn_deliver_to   0   //线程已经转交
#define cn_keep         1   //线程保留不删除
#define cn_delete       2   //线程应该被删除
#define cn_deleted      3   //线程已经被删除

//----事件逸出-----------------------------------------------------------------
//功能: 事件处理函数自然返回,其行为类似于djy_event_completed,对于独立型事件，这
//      两个函数是一致的，但对于关联型，则有所不同。djy_event_completed不会把事
//      件从事件队列中删除，也不会把线程置于初始态，djy_event_exit则会删除事件，
//      并把线程重新初始化。
//参数: 无
//返回: 本函数不返回
//-----------------------------------------------------------------------------
...
//会立即注销，只是不能弹出新事件，要等属于该事件类型的所有事件处理完才能注销。
    ...
    {
        m_free((void*)pg_event_running->vm);    //删除线程
        pl_evtt->vpus--;
        pg_event_running = pg_event_ready;
