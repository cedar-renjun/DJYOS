...
static struct pan_device tg_mem_of_device[cfg_device_limit];//泛设备控制块内存池
...
bool_t dev_delete_device(struct pan_device * device)
{
    ...
    if(device->left_semp != NULL)
        if(semp_query_capacital(device->left_semp) 
                    != semp_query_free(device->left_semp))
            en_del = false;
    if(device->right_semp != NULL)
        if(semp_query_capacital(device->right_semp)
                    != semp_query_free(device->right_semp))
            en_del = false;
    if(en_del)
    {
        node = &device->node;
...
#endif  //#if ((cfg_pan_driver == 1) && (cfg_run_mode != cn_mode_tiny))
