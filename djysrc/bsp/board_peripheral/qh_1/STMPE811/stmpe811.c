...
#if (cfg_single_touch == 1) &&(cfg_gui_kernel ==1)
...
ufast_t read_touch_stmpe811(struct single_touch_msg *touch_data);
...
//返回: 无
//-----------------------------------------------------------------------------
ptu32_t module_init_touch_stmpe811(ptu32_t para)
{
    static struct single_touch_private stmpe811;
    if(!touch_hard_init( ))
        return -1;
    touch_ratio_adjust((char*)para);
    stmpe811.read_touch = read_touch_stmpe811;
    touch_install_device("stmpe811",&stmpe811);
    return 1;
}
...
void touch_ratio_adjust(char *display_name)
{
    ...
    if((touch_init = djyfs_fopen("sys:\\touch_init.dat","r")) != NULL)
    {
        
        djyfs_fread(&tg_touch_adjust,sizeof(struct st_touch_adjust),1,touch_init);
    }
    else
    {
	...
	//    gk_destroy_win(desktop);
        ...
        djyfs_fwrite(&tg_touch_adjust,sizeof(struct st_touch_adjust),1,touch_init);
    }
