...
#define cn_fcco             (cfg_mclk* 3)    //  PLL频率(275Mhz~550Mhz)
#define cn_pll_n            1               //  1~256
#define cn_pll_m            ((cn_fcco / cfg_extclk) * (cn_pll_n) / 2)   //6~512
...
//外设时钟选择，在此全部设为相同的数值，该值在config-prj.h中定义
    ...
    pg_sysctrl_reg->cclkcfg = (cn_fcco / cfg_mclk) - 1;   
...
// 设置存储器加速模块
#if cfg_mclk <= 20000000                         //使用1个CPU时钟
    pg_sysctrl_reg->flashcfg = (((u32)0x01 << 12) & (~(0x003f))) | 0x003a;
#endif 

#if cfg_mclk > 20000000 && cfg_mclk <= 40000000   //使用2个CPU时钟
    pg_sysctrl_reg->flashcfg = (((u32)0x02 << 12) & (~(0x003f))) | 0x003a;
#endif

#if cfg_mclk > 40000000 && cfg_mclk <= 60000000   //使用3个CPU时钟
    pg_sysctrl_reg->flashcfg = (((u32)0x03 << 12) & (~(0x003f))) | 0x003a;
#endif

#if cfg_mclk > 60000000 && cfg_mclk <= 80000000   //使用4个CPU时钟
    pg_sysctrl_reg->flashcfg = (((u32)0x04 << 12) & (~(0x003f))) | 0x003a;
#endif

#if cfg_mclk > 80000000 && cfg_mclk <= 100000000  //使用5个CPU时钟
