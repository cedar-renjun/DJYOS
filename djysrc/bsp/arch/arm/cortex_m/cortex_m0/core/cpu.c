...
void __djy_set_delay(void)
{
    ...
    clksum = cfg_fclk/50;              //减计数,计满是100mS
    ...
    u32g_ns_of_u32for = (uint64_t)counter * 10E8/cfg_fclk/u32_fors;//防溢出，用64位
    ...
    counter = clksum - counter;    //取实际脉冲数。
    u32g_ns_of_u16for = (uint64_t)counter * 10E8/cfg_fclk/u16_fors;//防溢出，用64位
    ...
    counter = clksum - counter;    //取实际脉冲数。
    u32g_ns_of_u8for = (uint64_t)counter * 10E8/cfg_fclk /(u8_fors1*(u8_fors2+1));
...
void __djy_init_tick(void)
{
    ...
    pg_systick_reg->reload = cfg_fclk/cfg_tick_hz;
    pg_systick_reg->ctrl =   (1<<bo_systick_ctrl_enable)    //使能
...
u32 djy_get_fine_time(void)
{
    ...
    if(pg_systick_reg->ctrl & bm_systick_ctrl_countflag)
        time2 = (u32g_os_ticks+1)*cfg_tick_us;
    else
        time2 = u32g_os_ticks*cfg_tick_us
                    + ((pg_systick_reg->reload-time1)*cfg_fine_us >>16);
