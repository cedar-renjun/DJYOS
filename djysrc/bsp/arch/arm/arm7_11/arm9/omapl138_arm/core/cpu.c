...
//    pg_timer0_reg->TCR |= (1<<10);      //设置t64-0的t12为读清零模式
    ...
    u32g_ns_of_u32for = ((u64)cfg_fine_us * u32_fors>>16) / 10;
    u32g_ns_of_u16for = ((u64)cfg_fine_us * u16_fors>>16) / 10;
    u32g_ns_of_u8for  = ((u64)cfg_fine_us * u8_fors>>16) / 10;
...
void __djy_init_tick(void)
{
    ...
    timer_set_clksrc(0,0,0);            //设置晶振时钟作为定时器时钟
    timer_counter_len_set(0,1);         //设置为双32位计数
    timer_set_Period(0,0,cfg_fine_hz/cfg_tick_hz);   //t12周期值设为tick周期
...
uint32_t djy_get_fine_time(void)
{
    ...
    if(g_ptTimer0Reg->INTCTLSTAT & 0x02)
        ...
        time2 = u32g_os_ticks*cfg_tick_us + (time1*cfg_fine_us >>16);
