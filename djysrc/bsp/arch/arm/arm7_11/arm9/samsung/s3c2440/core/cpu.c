...
void __djy_set_delay(void)
{
    ...
    phase_ns = 10E8/cfg_timer_clk * 16;  //每脉冲的ns数
    clksum = (uint16_t)(cfg_timer_clk/16000);     //1000uS，减计数
...
void __djy_init_tick(void)
{
    ...
    timer_set_clk_source(3,0);          //pclk预分频数的1/2分频
    timer_set_precale(1,cfg_timer_clk/cfg_fine_hz/2 -1);
    //以上把定时器输入时钟的频率设为cn_fine_hz
    //timer_set_counter(3,(uint16_t)((u32)cn_tick_us*1000/cn_fine_ns));
    timer_set_counter(3, cfg_fine_hz/cfg_tick_hz);
...
u32 djy_get_fine_time(void)
{
    ...
    time1 = cfg_fine_hz/cfg_tick_hz - pg_timer_reg->TCNTO3;
