...
u32 uart0_int(ufast_t uart_int_line)
{
    ...
    if(pg_int_reg->SUBSRCPND & BIT_SUB_RXD0)
    {
        ...
        if( fifo_reg & cn_rx_over)
        {
            ...
            djy_event_pop(u16g_evtt_uart0_error,NULL,0,&parameter,
    ...
    if(pg_int_reg->SUBSRCPND & BIT_SUB_ERR0)
    {
        ...
        djy_event_pop(u16g_evtt_uart0_error,NULL,0,NULL,0,0,0);
    }
    return 0;
}
...
u32 uart1_int(ufast_t uart_int_line)
{
    ...
    if(pg_int_reg->SUBSRCPND & BIT_SUB_ERR1)
    {
        ...
        djy_event_pop(u16g_evtt_uart1_error,NULL,0,NULL,0,0,0);
    }
    return 0;
}
...
u32 uart2_int(ufast_t uart_int_line)
{
    ...
    if(pg_int_reg->SUBSRCPND & BIT_SUB_RXD2)
    {
        ...
        if( fifo_reg & cn_rx_over)
        {
            ...
            parameter = enum_recv_fifo_over;
            djy_event_pop(u16g_evtt_uart2_error,NULL,0,&parameter,
                                    sizeof(parameter),0,0);
        }
    }
    ...
    if(pg_int_reg->SUBSRCPND & BIT_SUB_ERR2)
    {
        ...
        djy_event_pop(u16g_evtt_uart2_error,NULL,0,NULL,0,0,0);
    }
    return 0;
}
                            ...
                            ptu32_t res2,ptu32_t len)
{
    ...
    if(recv_bytes != len)
    {
        ...
        djy_event_pop(error_evtt,NULL,0,&uart_error,sizeof(uart_error),0,0);
    }
    valid_bytes = ring_check(&uart_port->recv_ring_buf);
    if(valid_bytes >= uart_port->recv_trigger_level)
    {
        //如果不登记右手写事件，就只能查询方式从设备读取数据了。
        djy_event_pop(uart_port->evtt_right_write,NULL,0,NULL,0,0,0);
    }
                                        ...
                                        ptu32_t dst_buf,ptu32_t len)
{
    ...
    if(dst_buf == 0)
        return 0;
    uart_port = (struct uart_UCB*)dev_get_my_tag(uart_dev);
    if(ring_is_full(&uart_port->send_ring_buf))
        full = true;
    ...
    switch(cmd)
    {
        ...
        case cn_uart_set_baud:  //设置Baud
            if(uart_port->baud !=data1)
            {
                ...
                uart_port->my_reg->UBRDIV = ((cfg_pclk<<2)/data1 -32)>>6;
//                pg_uart1_reg->UBRDIV =((cfg_pclk<<2)/data -32)>>6;
            }
            break;
...
ptu32_t module_init_uart0(ptu32_t para)
{
    ...
    u32 temp;
    //保护缓冲区的信号量，使缓冲区中空闲为0时阻塞写入线程，读取线程使缓冲区中
    ...
    tg_uart0_CB.my_reg->UBRDIV = ((cfg_pclk<<2)/115200 -32)>>6;
    ...
    u16g_evtt_uart0_error = djy_evtt_regist(enum_correlative,cn_prio_real,0,0,5,
                                        uart0_error_service,1024,NULL);
    if(u16g_evtt_uart0_error == cn_invalid_evtt_id)
        goto exit_from_error_evtt;
    uart_send_evtt = djy_evtt_regist(enum_correlative,cn_prio_real,0,0,
...
ptu32_t module_init_uart1(ptu32_t para)
{
    ...
    tg_uart1_CB.my_reg->UBRDIV = ((cfg_pclk<<2)/115200 -32)>>6;
    ...
    u16g_evtt_uart1_error = djy_evtt_regist(enum_correlative,cn_prio_real,
                           0,0,5,uart1_error_service,1024,NULL);
...
ptu32_t module_init_uart2(ptu32_t para)
{
    ...
    tg_uart2_CB.my_reg->UBRDIV = ((cfg_pclk<<2)/115200 -32)>>6;
    ...
    u16g_evtt_uart2_error = djy_evtt_regist(enum_correlative,cn_prio_real,
                           0,0,5,uart2_error_service,1024,NULL);
