...
bool_t uart_set_baud(u32 uart_no,u32 baud)
{
    ...
    temp = cfg_pclk/16/baud;
    temp1 = cfg_pclk/16%baud;
    ...
    //如果误差 < cfg_pclk/16>>6 = 1/64，说明不用小数波特率误差已经很小。
    //第二个条件是17xx系列uart的要求，分配系数大于2才能用小数波特率
    if((err <= (cfg_pclk/16>>6)) || (temp <= 2))
    {
    ...
    for(divval = 1; divval < 15; divval++)
    {
        ...
        {
            temp1 = cfg_pclk * mulval / 16 / (mulval + divval) % baud;
            if(temp1 > (baud >> 1))
            {
                err = baud - temp1;
            }else
    ...
    if(result == true)
    {
        temp = (cfg_pclk * mulval / 16 / (mulval + divval) + baud -1) / baud;
        reg->LCR |= 1<<bo_uart_lcr_dlab;
        reg->RTD.DLL = temp;
        reg->DI.DLM = temp>>8;
...
ptu32_t module_init_uart0(ptu32_t para)
{
    ...
    gpio_cfg_pin_func(0,2,cn_p0_2_txd0);
    gpio_cfg_pin_func(0,3,cn_p0_3_rxd0);
...
ptu32_t module_init_uart1(ptu32_t para)
{
    ...
    gpio_cfg_pin_func(0,15,cn_p0_15_txd1);
    gpio_cfg_pin_func(0,16,cn_p0_16_rxd1);
