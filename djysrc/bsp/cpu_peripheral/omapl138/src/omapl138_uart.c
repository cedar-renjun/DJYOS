...
void __uart_set_baud(tag_UartReg volatile *reg,u32 baud)
{
    ...
    {
        temp = (u32)(cfg_pll0_sysclk2+(baud>>1))/(baud*16);
        reg->DLL = (u8)temp;
        reg->DLH = (u8)(temp>>8);
    }else                               //串口1和串口2
    {
        ...
        {
            temp = (u32)(cfg_pll1_sysclk2+(baud>>1))/(baud*16);
            reg->DLL = (u8)temp;
            reg->DLH = (u8)(temp>>8);
        }else
        {
...
//    tg_uart0_CB.my_reg->IIR_FCR |= 1;   //set FIFOEN
    ...
    u16g_evtt_uart0_error = djy_evtt_regist(
                           enum_correlative,cn_prio_real,0,0,5,
                            ...
                            ptu32_t res2,ptu32_t len)
{
    ...
    if(recv_bytes != len)
    {
        ...
        djy_event_pop(error_evtt,NULL,0,&uart_error,
                            ...
                            ptu32_t res,ptu32_t len)
{
    ...
    while(1)
    {
        ...
        djy_event_pop(uart_port->evtt_left_write,NULL,0,
